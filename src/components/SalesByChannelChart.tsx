...
interface Props {
  data: { date: string; shopify: number; amazon: number; walmart?: number }[]
}
...
export default function SalesByChannelChart({ data }: Props) {
  ...
  const hasWalmart = data.some(d => (d.walmart ?? 0) > 0)
  const topBar = hasWalmart ? 'walmart' : hasAmazon ? 'amazon' : 'shopify'
