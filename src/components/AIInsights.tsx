...
export default function AIInsights({ metrics, period, preset, orgName, platform = 'ecommerce' }: Props) {
  ...
  return (
    <div style={{ background: '#000', border: '1px solid #222', borderRadius: 10, padding: 24, marginBottom: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: generated ? 16 : 0 }}>
        ...
        <button
          onClick={generate}
          disabled={loading}
          style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', background: loading ? 'rgba(255,255,255,0.05)' : '#00ff97', color: loading ? 'rgba(255,255,255,0.4)' : '#000', border: 'none', borderRadius: 6, fontFamily: 'Barlow, sans-serif', fontWeight: 700, fontSize: '0.875rem', cursor: loading ? 'not-allowed' : 'pointer', transition: '0.15s' }}
        >
          {loading ? (
      ...
      {!loading && insight && (
        <p style={{ fontSize: '0.9375rem', color: '#fff', fontFamily: 'Barlow, sans-serif', lineHeight: 1.7, margin: 0 }}>
          {insight}
        </p>
      )}

      {!loading && !insight && (
        <p style={{ fontSize: '0.875rem', color: 'rgba(255,255,255,0.5)', fontFamily: 'Barlow, sans-serif', marginTop: 12, margin: 0 }}>
          Click "Generate insights" to get an AI-powered analysis of this period's performance.
        </p>
      )}
    </div>
  )
